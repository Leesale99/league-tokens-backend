...
4. Only after PR creation succeeds, obtain the board item with `scripts/issue-workflow/get_item.sh $1` (status-agnostic lookup), move it to In review, remove `ready-for-agent`, and add `ready-for-human` using the workflow scripts.
