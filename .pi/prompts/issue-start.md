...
5. Do NOT create any branch here — the feature branch `feat/<issue>-<slug>` is created by `scripts/issue-workflow/v2/worktree.sh <issue> <slug>` at implement time. (The retired v1 `create_branch.sh` switched the main checkout onto the feature branch, which broke the worktree and polluted main — never do that.)
