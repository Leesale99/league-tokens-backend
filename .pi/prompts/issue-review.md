...
3. Report the printed `tmux attach` command. Explain that the orchestrator will read every issue document, dispatch the five independent reviewer role contracts from `scripts/issue-workflow/v2/roles/`, collect their reports under `docs/issue-workflows/$1/reviews/`, and wait for findings to be fixed and re-reviewed.
