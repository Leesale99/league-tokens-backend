...
1. Read the run's track: `jq -r '.track' docs/issue-workflows/$1/workflow.json`. **Track S**: skip the review gate — the CI pipeline is the only review gate. Otherwise run `scripts/issue-workflow/check_review_gate.sh $1`; if it exits non-zero, report its message and stop — the gate is mechanical; do not override it with your own judgement of the findings. Then read `docs/issue-workflows/$1/issue.md` and `plan.md` (Track S has no `plan.md` — use the task brief) for the PR title and summary.
