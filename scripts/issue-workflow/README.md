...
The prompt templates in `.pi/prompts/` (`/issue-*` commands) use these scripts
for GitHub-board actions, branch creation, and tmux-backed context/review
sessions. Role contracts for the v2 workflow live in
`scripts/issue-workflow/v2/roles/` (plain files, not slash commands).

- `get_item.sh`, `pick_ready.sh`, `set_status.sh`, `add_label.sh`, `remove_label.sh`: GitHub Project and issue-label operations. `get_item.sh <N>` is a status-agnostic board lookup (prints number/title/item_id/status; `--expect <status>` asserts it); `pick_ready.sh` picks the first Ready item.
...
- `v2/`: Phase 0+ artifacts — sandbox template (`templates/base.Dockerfile`), spike runners (`spike/`), role contracts (`roles/`).
