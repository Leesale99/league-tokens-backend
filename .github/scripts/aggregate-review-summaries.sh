...
# Manages the code-review-passed label.
set -euo pipefail
...
if [ "$HAS_GO" = "true" ]; then
  ...
  # ── Apply code-review-passed label ──────────────────────────────────

  if [ "$total_blocking" -eq 0 ]; then
    gh pr edit "$PR_NUMBER" --repo "$REPO" --add-label code-review-passed 2>/dev/null || \
      echo "::warning::Could not add code-review-passed label (may not exist in repo)"
  else
    gh pr edit "$PR_NUMBER" --repo "$REPO" --remove-label code-review-passed 2>/dev/null || true
  fi
else
  echo "No Go files changed — skipping review summary, removing code-review-passed."
  gh pr edit "$PR_NUMBER" --repo "$REPO" --remove-label code-review-passed 2>/dev/null || true
fi
