...
The body field of create_pull_request_review must be `## Correctness Review`
...
- You have something new to add → reply to that existing thread with
  `➡️ **Also flagged by Correctness**: <your finding>`. Use `create_pull_request_review`
...
- If another focus said the same thing with a different severity, mention:
  `➡️ **Also flagged by Correctness** (🔴 BLOCKING): <brief restatement>`.
